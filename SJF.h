...
public:
    bool operator() (Process const *p1, Process const *p2) {
        if (p1->CPUBursts.at(0) < p2->CPUBursts.at(0)) {
            return true;
        }

        else if (p1->CPUBursts.at(0) > p2->CPUBursts.at(0)) {
            return false;
        }
        else {
            if (p1->name <= p2->name) {
...
void SJF(deque<Process*> processes, double tau, int t_cs, double alpha) {
    ...
    while (true) {
        ...
        if (!CPUActive) {
            ...
            if (readyQ.size() > 0 && (readyQ.at(0)->time_for_next_interesting_event == clock ||
                ...
                printf("time %dms: Process %c started using the CPU for %dms burst ", clock,
                       cpuInUse->name, cpuInUse->CPUBursts[0]);
                // update releaseCPU
                printQueue(readyQ);
        ...
        if (CPUActive) {
            ...
            if (cpuInUse->time_for_next_interesting_event == clock) {
                ...
                if (cpuInUse->CPUBursts.size() > 0) {
                    ...
                    printQueue(readyQ);
                    int newTau = calc_tau(cpuInUse->tau, clock, alpha);
        ...
        if(it != waitingMap.end()) {
            ...
            for (int i = 0; i < it->second.size(); i++) {
                ...
                if(cpuInUse != NULL || (readyQ.size() != 0 && temp2 != readyQ.at(0))) {
                    temp2->time_for_next_interesting_event = clock + t_cs;
                } else {
