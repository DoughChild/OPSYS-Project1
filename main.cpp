...
void printProcesses(deque<Process*> processes) {
    vector<Process*> p;
    for (unsigned long i = 0; i < processes.size(); i++) {
        if (processes[i]->num_bursts == 1) {
            printf("Process %c (arrival time %d ms) %d CPU burst (tau %dms)\n", processes[i]->name, processes[i]->t_arrival,
                   processes[i]->num_bursts, processes[i]->tau);
        }
        else {
        ...
        for (int j = 0; j < processes[i]->num_bursts - 1; j++) {
            printf("--> CPU burst %d ms --> I/O burst %d ms\n", processes[i]->CPUBursts[j], processes[i]->IOBursts[j]);
        }
        printf("--> CPU burst %d ms\n", processes[i]->CPUBursts[processes[i]->num_bursts - 1]);
    }
}
...
//        printf("length of CPU bursts: %lu, length of I/O bursts: %lu\n", processes[1]->CPUBursts.size(), processes[1]->IOBursts.size());
        if (i == 0) {
            ...
            SJF(processes, t_cs, alpha);
            cout << "" << endl;
        }
        else if (i == 2) {
            SRT(processes, t_cs, alpha);
            cout << "" << endl;
        }
        else {
            RR(processes, t_cs, alpha, t_slice);
        }
